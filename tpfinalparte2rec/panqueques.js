class Panqueques {
   ...
   reiniciar() {
    ...
    verificarColision(jugadorX, estadoRef, jugadorRef) {
        
        let alturaTorre = estadoRef.pilaPanqueques.length * this.alto;
        let alturaColisionY = jugadorRef.y - jugadorRef.alto / 2 - alturaTorre;
        ...
        if (this.y >= alturaColisionY) {
            if (this.x > jugadorX - this.ancho / 2 && this.x < jugadorX + this.ancho / 2) {
              ...
              if (this.tipo === 'ROJO') { 
                ...
                estadoRef.apilar(this.color, this.alto); 
                this.reiniciar();
            }
            return true;
